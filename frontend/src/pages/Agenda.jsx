...
const C = {
  ...
  wait: "#fbbf24",
  waitBg: "rgba(251,191,36,.12)",
  waitBd: "rgba(251,191,36,.32)",
  can: "#f87171",
...
const evTop = (s) => {
  ...
  return Math.max(
    0,
    ((d.getHours() * 60 + d.getMinutes() - START_H * 60) / 60) * HOUR_H,
  );
};
const evHpx = (s, e) => {
  if (!s || !e) return 28;
  return Math.max(((new Date(e) - new Date(s)) / 60000 / 60) * HOUR_H - 4, 28);
};
const nowPx = () => {
  const n = new Date();
  return ((n.getHours() * 60 + n.getMinutes() - START_H * 60) / 60) * HOUR_H;
};
...
function freeSlots(turnos) {
  const v = turnos
    ...
    .map((t) => ({
      s:
        new Date(t.fecha_inicio).getHours() * 60 +
        new Date(t.fecha_inicio).getMinutes(),
      e:
        new Date(t.fecha_fin).getHours() * 60 +
        new Date(t.fecha_fin).getMinutes(),
    }))
    .sort((a, b) => a.s - b.s);
  const slots = [];
  let cur = START_H * 60;
  for (const { s, e } of v) {
    if (s - cur >= 45) slots.push({ s: cur, e: s, dur: s - cur });
    cur = Math.max(cur, e);
  }
  if (END_H * 60 - cur >= 45)
    slots.push({ s: cur, e: END_H * 60, dur: END_H * 60 - cur });
  return slots;
}
        ...
        {items.map(({ l, p, active }) => (
          <button
            ...
            style={{
              ...
              boxShadow: active ? `inset 0 0 0 1px ${C.borderStrong}` : "none",
              border: "none",
...
function Gutter() {
  return (
    <div
      style={{ width: 58, flexShrink: 0, borderRight: `1px solid ${C.border}` }}
    >
      {HOURS.map((h) => (
...
function EvCard({ t, onEdit, compact }) {
  ...
  return (
    <div
      ...
      style={{
        position: "absolute",
        top: evTop(t.fecha_inicio),
        height: evHpx(t.fecha_inicio, t.fecha_fin),
        left: compact ? 3 : 5,
        right: compact ? 3 : 5,
        borderRadius: 9,
        padding: compact ? "4px 6px 4px 9px" : "6px 9px 6px 12px",
        background: c.bg,
...
function FreeSlot({ slot }) {
  const hS = Math.floor(slot.s / 60),
    mS = slot.s % 60,
    hE = Math.floor(slot.e / 60),
    mE = slot.e % 60;
  return (
    <div
      style={{
        position: "absolute",
        top: ((slot.s - START_H * 60) / 60) * HOUR_H,
        height: Math.max((slot.dur / 60) * HOUR_H - 4, 22),
        left: 5,
        right: 5,
        zIndex: 1,
        borderRadius: 7,
        padding: "4px 10px",
        background: `repeating-linear-gradient(135deg,${C.aiBg} 0 8px,transparent 8px 16px)`,
        border: `1px dashed ${C.aiBd}`,
        display: "flex",
        alignItems: "center",
        gap: 7,
        overflow: "hidden",
    ...
    >
      ...
      <span style={{ color: C.textDim, fontSize: 11, whiteSpace: "nowrap" }}>
        {p2(hS)}:{p2(mS)} – {p2(hE)}:{p2(mE)} · {slot.dur} min libres
      </span>
    </div>
  );
}
...
function DayView({ turnos, date, onEdit }) {
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", borderBottom: `1px solid ${C.border}` }}>
        ...
        <div
          style={{
            flex: 1,
            padding: "14px 20px",
            background: isToday(date) ? "rgba(56,189,248,.05)" : "transparent",
          }}
        >
          <div
            style={{
          ...
          >
            <span
              style={{
                ...
                fontWeight: 800,
                letterSpacing: -1,
                color: isToday(date) ? C.accent : C.text,
                fontVariantNumeric: "tabular-nums",
              }}
            >
              {date.getDate()}
            ...
            <span style={{ fontSize: 16, color: C.textDim, fontWeight: 600 }}>
              {MES[date.getMonth()]} {date.getFullYear()}
            </span>
          </div>
          <div style={{ fontSize: 12, color: C.textDim, marginTop: 5 }}>
            ...
            · {turnos.filter((t) => t.estado === "pendiente").length} en espera
          </div>
        </div>
      </div>
      {/* Timeline */}
      <div
        className="ag-scroll"
        style={{ display: "flex", maxHeight: 580, overflowY: "auto" }}
      >
        <Gutter />
        <div
          style={{
            ...
            position: "relative",
            height: TOTAL_H,
            background: isToday(date) ? "rgba(56,189,248,.015)" : "transparent",
          }}
        >
          <HLines />
          {isToday(date) && <NowLine />}
          {slots.map((s, i) => (
            <FreeSlot key={i} slot={s} />
          ))}
          {turnos
            .filter((t) => t.fecha_inicio)
            .map((t) => (
              <EvCard key={t.id} t={t} onEdit={onEdit} />
            ))}
          {turnos.length === 0 && (
            <div
              style={{
          ...
          return (
            <div
              ...
              style={{
                ...
                background: today ? "rgba(56,189,248,.06)" : "transparent",
                cursor: "pointer",
                transition: ".12s",
              }}
            >
          ...
          return (
            ...
            >
              ...
              {ts
                .filter((t) => t.fecha_inicio)
                .map((t) => (
                  <EvCard key={t.id} t={t} onEdit={onEdit} compact />
                ))}
            </div>
          );
        })}
...
function MonthView({ turnosByDate, currentDate, onDayClick }) {
  ...
  const totalCells =
    dowIdx(firstDay) + lastDay.getDate() + (lastDow < 6 ? 6 - lastDow : 0);
          ...
          return (
            <div
              ...
              style={{
                ...
                background: today ? "rgba(56,189,248,.05)" : "transparent",
                cursor: "pointer",
                transition: ".12s",
                display: "flex",
            ...
            >
              ...
              {/* Loading hint */}
              {ts === undefined && inMonth && (
                <div style={{ fontSize: 10, color: C.textMute }}>···</div>
              )}
              {/* Event chips */}
              {chips.map((t, ci) => {
                const c = stCfg(t.estado);
              ...
              {extra > 0 && (
                <div
                  style={{ fontSize: 10, color: C.textMute, fontWeight: 700 }}
                >
                  +{extra} más
                </div>
              )}
...
export default function Agenda() {
  ...
  useEffect(() => {
    ...
    const load = async (dates) => {
      ...
      try {
        ...
        if (!cancelled)
          setError(
            e.response?.data?.detail || e.message || "Error al cargar turnos.",
          );
      } finally {
  ...
  const rangeLabel = () => {
    if (view === "day")
      return `${DOWL[dowIdx(currentDate)]} ${currentDate.getDate()}, ${MES[currentDate.getMonth()]}`;
    if (view === "week") {
      const mon = monday(currentDate),
        sun = addD(mon, 6);
  ...
  return (
    ...
    >
      ...
      <div
        style={{ maxWidth: 1380, margin: "0 auto", padding: "28px 28px 60px" }}
      >
              ...
              {todayCount !== undefined && (
                <>
                  ...
                  <b style={{ color: C.text }}>
                    {todayCount} turno{todayCount !== 1 ? "s" : ""}
                  </b>{" "}
                  programados hoy
                </>
              )}
            ...
            {[
              ...
              <button
                ...
                style={{
                  ...
                  fontWeight: 700,
                  fontSize: 13,
                  color: view === v ? "#04263f" : C.textDim,
                  background:
                    view === v
                      ? `linear-gradient(150deg,${C.accent},${C.accent2})`
                      : "transparent",
                  boxShadow:
                    view === v ? `0 3px 10px rgba(43,130,246,.3)` : "none",
                  border: "none",
                  cursor: "pointer",
                  transition: ".15s",
                }}
        ...
        >
          ...
          {view === "day" && (
            <DayView
              turnos={turnosByDate[toISO(currentDate)] || []}
              date={currentDate}
              onEdit={onEventEdit}
            />
          )}
